...
from paddle_bert_base import s4_preprocess_4core,mf_s4_infer_4core,s4_postprocess,pad_input
import random
import time
import math
...
def parse_args():
    ...
    parser.add_argument("--s4_model_path", type=str, help="the path of onnx model")
...
def evaluate(model, loss_fct, metric, data_loader):
    ...
    for batch in data_loader:
        ...
        # # print("%i,logits shape:%s,input_ids shape:%s"%(i,logits.shape,batch["input_ids"].shape))
        # i=i+1exi
        input_ids_data,token_type_ids_data = batch["input_ids"], batch["token_type_ids"]
        # if batch["input_ids"].shape[0]<32:
        #     # input_data_pad = np.pad(input_data,((0,0),(0,128-input_shape[1])),mode="constant",constant_values=(0))
        #     import pdb;pdb.set_trace()
        #     input_ids_data = pad_input(input_ids_data)
        #     token_type_ids_data = pad_input(token_type_ids_data)
        #     print("no enough data, need to upatae model later")
        #     continue
        # if batch["input_ids"].shape[0]>=32:continue
        input_ids_tensor,token_type_ids_tensor,input_ids_2_tensor = s4_preprocess_4core(batch["input_ids"], batch["token_type_ids"])
        ...
        # logits = mf_s4_infer_4core(model_path,input_ids_tensor,token_type_ids_tensor,input_ids_2_tensor)
        # model_path = '/home/moffett/work/zhongming/project/paddle_compile_output/bert_base_uncased_sst2_9266/compile_output_1222/backend/paddle_bert_base_uncased#N8#128_change_placeholder/model.bin'
        model_path = args.s4_model_path
        logits = mf_s4_infer_4core(model_path,input_ids_2_tensor,input_ids_tensor,token_type_ids_tensor)
        logits = s4_postprocess(logits)
        if batch["input_ids"].shape[0]<32:
            logits=logits[0:batch["input_ids"].shape[0],:]
